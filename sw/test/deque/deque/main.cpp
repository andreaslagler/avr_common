...
Deque<Test>::allocator_type::size_type checkAllocator()
{
    Deque<Test>::allocator_type allocator;
    for (Deque<Test>::allocator_type::size_type len = HEAP_SIZE; len > 0; --len)
    {
...
int main(void)
{
    ...
    const Deque<Test> testDeque(testInit);
    ...
    
    {
        testPassed = true;
        Test::resetCounter();
        Deque<Test> x(testInit.begin(), testInit.end());
...

    {
        ...
        Deque<Test> x(testInit);
        auto it = testInit.begin();
        for (Deque<Test>::size_type idx = 0; idx < x.size(); ++idx)
        {
...

    {
        ...
        Test::resetCounter();
        Deque<Test> x(testDeque);
        x.emplaceFront(testDeque.front().getValue());
    ...
    testPassed &= (len == checkAllocator());
    allPassed &= test_assert("emplaceFront()", testPassed && Test::check(0,1,6,0,7));
    ...
    testPassed &= (len == checkAllocator());
    allPassed &= test_assert("pushFront()", testPassed && Test::check(0,0,7,0,7));

    {
        testPassed = true;
        Test::resetCounter();
        Deque<Test> x(testDeque);
        Test t = testDeque.front();
        x.pushFront(move(t));
        testPassed &= (x.size() == (testDeque.size()+1));
        auto it = x.begin();
        testPassed &= (*it).getValue() == testDeque.front().getValue();
    ...
    testPassed &= (len == checkAllocator());
    allPassed &= test_assert("pushFront()", testPassed && Test::check(0,0,7,1,8));
